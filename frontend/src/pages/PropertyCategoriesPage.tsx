...
import { PROPERTY_CATEGORIES, type PropertyCategory } from "@/data/propertyCategoriesData";
...
export default function PropertyCategoriesPage() {
  const [data, setData]                 = useState(PROPERTY_CATEGORIES);
  ...
  function openDialog() { setForm(empty); setErrors({}); setOpen(true); }

  function handleSubmit() {
    if (!form.name.trim()) { setErrors({ name: "Name is required" }); return; }
    const now = new Date().toISOString();
    const newEntry: PropertyCategory = {
      id: `pc-${Date.now()}`, name: form.name.trim(),
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="relative">
          ...
          <Input placeholder="Search categories..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-9 w-56 text-sm" />
        </div>
        <div className="flex-1" />
        <p className="text-sm text-muted-foreground">{filtered.length} categor{filtered.length !== 1 ? "ies" : "y"}</p>
        <DropdownMenu>
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              <tr key={c.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-3 text-muted-foreground max-w-sm"><span className="line-clamp-2">{c.description}</span></td>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Add Property Category</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label htmlFor="pc-name">Name <span className="text-destructive">*</span></Label>
              <Input id="pc-name" placeholder="e.g. Residential, Commercial, Land / Plot"
                value={form.name} onChange={(e) => { setForm((f) => ({ ...f, name: e.target.value })); setErrors({}); }} />
              {errors.name && <p className="text-xs text-destructive">{errors.name}</p>}
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="pc-desc">Description</Label>
              <Textarea id="pc-desc" placeholder="Brief description of this category..." rows={3}
                value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} />
            </div>
            <div className="flex items-center justify-between">
              <Label htmlFor="pc-active">Is Active</Label>
              <div className="flex items-center gap-2">
                <Switch id="pc-active" checked={form.isActive} onCheckedChange={(v) => setForm((f) => ({ ...f, isActive: v }))} />
